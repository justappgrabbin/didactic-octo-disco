...
export default function App() {
  ...
  useEffect(() => { void loadNodes().then(setNodes); }, []);
  ...
  function move(id: string, x: number, y: number) {
    setNodes(prev => { const next = prev.map(n => n.id === id ? { ...n, x, y, updatedAt: new Date().toISOString() } : n); saveLocalNodes(next); const node = next.find(n => n.id === id); if (node) void saveNode(node).catch(() => setSyncState('local')); return next; });
  }
  function addIntent(text: string) { ingest([parseIntentToNode(text, { x: 320 + Math.random() * 120, y: 180 + Math.random() * 80 })]); }
